...
class Application extends Slim
{
    ...
    public function __construct(array $userSettings = array(), $configDirectory = 'config')
    {
        ...
        $this->get('/querysql/:id/:sel', function ($id, $sel) {
            $queryphp = new QuerySQL($this->config['connMySQL'], $this->config['queries']);
            $query = $queryphp->queryJson($id, $sel);
            if ($query === null) {
                return $this->notFound();
            }
            $this->response->headers->set('Cache-Control', 'no-cache, must-revalidate');
            $this->response->headers->set('Content-Type', 'application/json');
            $this->response->setBody(json_encode($query));
        });
